...
public class Office2Swf 
{
	...
	private static String getCommand(String pdfFile, String swfOutFilePath)
	{
		...
		{
			command = "pdf2swf -s languagedir=/usr/local/xpdf-chinese-simplified -T 9 " + pdfFile + " -o " + swfOutFilePath;
		} 
		else if (Pattern.matches("Windows.*", osName)) 
		{
			command = "C:/SWFTools/pdf2swf.exe -t " + pdfFile + " -o "  + swfOutFilePath + " -T 9";
//			command = "D:/SWFTools/pdf2swf.exe -t " + pdfFile + " -o "  + swfOutFilePath + " -T 9";
		} 
	...
	public static String pdf2Swf(String pdfInputFilePath, String swfOutFilePath)
	{
		...
		try
		{
			...
			pro.waitFor();
			return swfOutFilePath.replaceAll("." + getPostfix(swfOutFilePath), ".swf");
		}
		catch(Exception ex)
		{
			ex.printStackTrace();
