package com.ibb.dao.depart.impl;
...
@Repository("departDicDao")
public class DepartDicDao extends BaseHibernateDao<DepartDicInfo, Integer> implements IDepartDicDao{
	...
	public DepartDicInfo get(String dicNum) {
		DepartDicInfo departDicInfo = null;
		...
		if (StringUtils.isNotBlank(dicNum)) {
			...
			List<DepartDicInfo> lists = this.queryListByCondition(conditionQuery);
			if (lists.size() > 0) {
				departDicInfo = lists.get(0);
			}
		} else {
			throw new IbbMgrException("字典编码不能为空");
		}

		return departDicInfo;
	...
	public List<DepartDicInfo> findNodes(String upSystemId) {
		...
		if (StringUtils.isNull(upSystemId)) {
			hql = "from DepartDicInfo dic where dic.operId='' or dic.operId is null ";
		} else {
			hql = "from DepartDicInfo dic where dic.operId= :operId";
	...
	public String findMaxSystemId(String upSystemId) {
		String hql = "Select max(dic.dicNum) from DepartDicInfo dic where dic.operId=:upSystemId";
		Map<String, Object> params = new HashMap<String, Object>();
	...
	public DepartDicInfo findByNameAndParentName(String name, String pname) {
		Map<String, Object> params = new HashMap<String, Object>();
		String hql = "select t from DepartDicInfo t,DepartDicInfo tp where t.operId=tp.dicNum and t.dicName= :name and tp.dicName= :pname";
