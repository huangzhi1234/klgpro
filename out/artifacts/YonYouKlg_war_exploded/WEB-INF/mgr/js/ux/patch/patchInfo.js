...
$(function() {
	...
	// 初始化表格
	var patchInfoTable = {
		url : ctx + '/mgr/patchInfo/list.json',
		columns : [ [{
			field : 'patId',
			width : 20,
			align : 'left',
	...
	//新增按钮  	 项目基本信息
	$("#btnAdd_patchInfo").click(function() {
	...
	//编辑按钮
	$("#btnEdit_patchInfo").click(function() {
		//取得选中的数据
	...
	$('#btnWatch_patchInfo').click(function() {
		//取得选中的数据
		var arr = $('#datagridOfParchInfo').datagrid("getSelections");
		if (arr.length == 1) {
	...
	//删除
	$("#btnDelete_patchInfo").click(function() {
		var arr = $('#datagridOfParchInfo').datagrid("getSelections");
		if (arr.length > 0) {
	...
	//批量申请
	$("#btnApply_patchInfo").click(function() {
		var arr = $('#datagridOfParchInfo').datagrid("getSelections");
		var flag=0;
		var patIdArr = '';
		var idArr = 'idArr=';
