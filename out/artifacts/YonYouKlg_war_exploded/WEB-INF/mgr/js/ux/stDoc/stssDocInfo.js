...
$(function(){
		...
		// 初始化表格
		var stDocInfoTable = {
			url : ctx + '/mgr/stssDocInfo/list.json',
			columns : [ [ {
				field : 'stdocId',
				width : 20,
				align : 'left',
			...
			if (arr.length > 0) {
				$.messager.confirm('确认', '您确认想要删除选中的记录吗？', function(r) {
					if (r) {
						...
						$.ajax({
							type : 'post',
							url : path + '/stssDocInfo/delete.json',
							data : stdocIdArr,
							success : function(data) {
								if (data.result == true) {
...
//						 window.open("../readonline/tpreadonline.jsp?url="+fileAddress);
//						 return;
//					}
		...
		$('#fileDown2').click(function() {
			...
			if(a=="下载"){
				var flag="b";
				location.href = path + "/stssDocInfo/downloadFile?fileAddress=" + fileAddress + "&stdocId=" + stdocId+"&flag="+flag;
			}else{
				/*var isOk=0;*/
				var param={};
...
//							$.ajax({
//								type:'post',
//								url:path+'/stdocDowncheck/getIsOk2',
//								data:param,
//								success:function(data){
...
//						$.ajax({
//							type : 'post',
//							url : path + '/stssDocInfo/applyMore.json',
//							data : stdocIdArr,
//							success : function(data) {
//								if (data.result == true) {
//									$.messager.alert('提醒', data.msg);
