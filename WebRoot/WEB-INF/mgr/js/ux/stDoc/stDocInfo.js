...
$(function(){
		...
		// 初始化表格
		var stDocInfoTable = {
			url : ctx + '/mgr/stDocInfo/list.json',
			columns : [ [ {
				field : 'stdocId',
				width : 20,
				align : 'left',
			...
			if (arr.length > 0) {
				$.messager.confirm('确认', '您确认想要删除选中的记录吗？', function(r) {
					if (r) {
						...
						$.ajax({
							type : 'post',
							url : path + '/stDocInfo/delete.json',
							data : stdocIdArr,
							success : function(data) {
								if (data.result == true) {
...
//					var fileAddressess = arr[0].fileAddress.split(".");
//					if(fileAddressess[fileAddressess.length-1]=="jpeg"){
////						$("#imgss").attr('src', fileAddress);
//						$("#imgss").attr("src", path + "/readonline/getBomImg?address=" + fileAddress);
//						$("#pic").show();
//						// window.open("/YonYouKlg/mgr/jsp/readonline/tpreadonline.jsp?yulanUrl="+address);
//						 return;
//					}
					var url = path + "/readonline/read?filePath="+arr[0].fileAddress;
					if (fileName) {
...
//							$.ajax({
//								type:'post',
//								url:path+'/stdocDowncheck/getIsOk',
//								data:param,
//								success:function(data){
...
//						$.ajax({
//							type : 'post',
//							url : path + '/stDocInfo/applyMore.json',
//							data : stdocIdArr,
//							success : function(data) {
//								if (data.result == true) {
//									$.messager.alert('提醒', data.msg);
