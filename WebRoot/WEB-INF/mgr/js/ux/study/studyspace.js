...
$(function(){
		...
		// 初始化表格
		var stDocInfoTable = {
			url : ctx + '/mgr/studyspace/list.json',
			columns : [ [ {
				field : 'stdocId',
				width : 20,
				align : 'left',
			...
			if (arr.length > 0) {
				$.messager.confirm('确认', '您确认想要删除选中的记录吗？', function(r) {
					if (r) {
						...
						$.ajax({
							type : 'post',
							url : path + '/studyspace/delete.json',
							data : stdocIdArr,
							success : function(data) {
								if (data.result == true) {
...
//				}
		...
		$('#fileDown2').click(function() {
			...
			if(a=="下载"){
				var flag="b";
				location.href = path + "/studyspace/downloadFile?fileAddress=" + fileAddress + "&stdocId=" + stdocId+"&flag="+flag;
			}
		});
